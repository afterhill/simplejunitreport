<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:java="http://xml.apache.org/xslt/java" xmlns:stringutils="xalan://org.apache.tools.ant.util.StringUtils"
	...
	<xsl:template match="screenshot">
		...
		<xsl:variable name="screenshot.fullpath">
			...
			<xsl:text>/</xsl:text>
			<xsl:value-of select="$package.dir" />
			<xsl:text>/</xsl:text>
		...
		<xsl:value-of
			select="java:simple.junit.reporter.ImageHandler.decodeBase64AndSaveAsPng(string($encoded.image), string($screenshot.fullpath))" />
		<div>
			<br />
			<a>
